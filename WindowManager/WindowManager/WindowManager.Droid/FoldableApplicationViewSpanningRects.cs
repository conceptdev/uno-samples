...
namespace WindowManager.Droid
{
    ...
    public class FoldableApplicationViewSpanningRects : IApplicationViewSpanningRects, INativeDualScreenProvider, INativeFoldableProvider
	{
		...
		public IReadOnlyList<Rect> GetSpanningRects()
		{
			...
			{
				if (currentActivity.IsSeparating)
				{
					...
					List<Rect> spanningRects;

					if (currentActivity.FoldOrientation == AndroidX.Window.Layout.FoldingFeatureOrientation.Vertical)
					{
						spanningRects = new List<Rect>
						{
							// left region
							new Rect(0, 0, currentActivity.FoldBounds.Left, currentActivity.FoldBounds.Height()),
							// right region
							new Rect(currentActivity.FoldBounds.Left + currentActivity.FoldBounds.Width(),
								0,
								wuxWindowBounds.Width,
								wuxWindowBounds.Height),
						};
					}
					else 
					{
						...
						{
							// top region
							new Rect(0, 0, currentActivity.FoldBounds.Width(), currentActivity.FoldBounds.Top),
							// bottom region
							new Rect(0,
								currentActivity.FoldBounds.Bottom,
								wuxWindowBounds.Width,
								wuxWindowBounds.Height),
						};
					}
					return spanningRects;
				}
